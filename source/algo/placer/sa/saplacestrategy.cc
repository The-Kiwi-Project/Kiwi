...
namespace kiwi::algo {
    ...
    auto SAPlaceStrategy::place(
        ...
        auto total_cost = this->total_net_cost(nets) * this->_wirelength_weight + 
                          this->congestion_cost(interposer) * this->_congestion_weight +
                          this->thermal_cost(topdies) * this->_thermal_weight;
            ...
            for (std::usize n = 0; n < this->_solve_number; ++n) {
                ...
                if (this->decide_to_swap_topdie_inst(temperature)) {
                    ...
                    if (deltaCost <= 0) {
                        ...
                        if (random_f64() <= std::exp(-1.0 * (deltaCost) / temperature)) {
                            total_cost = new_total_cost;
                        } else {
        ...
        if (!this->is_valid_placement(interposer, topdies)) {
            debug::warning("There are issues with the final layout.");
        }
    ...
    auto SAPlaceStrategy::evaluate_placement(
        ...
        auto total_cost = wirelength * this->_wirelength_weight + 
                          congestion * this->_congestion_weight +
                          thermal * this->_thermal_weight +
                          power * this->_power_weight;
                          
        debug::info_fmt("Layout evaluation - Line length cost: {}, congestion cost: {}, thermal distribution cost: {}, power consumption cost: {}, total cost: {}", 
            wirelength, congestion, thermal, power, total_cost);
    ...
    auto SAPlaceStrategy::is_valid_placement(
        ...
        for (const auto& topdie : topdies) {
            ...
            if (used_tobs.contains(tob)) {
                debug::error_fmt("TOB {} is occupied by multiple chips", tob->coord().to_string());
                return false;
            }
            used_tobs.insert(tob);
        }

        auto interposer_rows = interposer->rows();
        auto interposer_cols = interposer->cols();
