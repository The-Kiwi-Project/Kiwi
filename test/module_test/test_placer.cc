...
static void test_basic_placement() {
    ...
    debug::debug("constructing topdies ...");
    std::Vector<TopDieInstance*> topdieinstace_ptrs;
    
    auto topdie1 = basedie.add_topdie("topdie1", std::HashMap<std::String, std::usize>{});
    auto tob1 = hardware::TOB(0, 0);
    auto topdieinst1 = basedie.add_topdie_inst("topdieinst1", topdie1, &tob1);

    auto topdie2 = basedie.add_topdie("topdie2", std::HashMap<std::String, std::usize>{});
    auto tob2 = hardware::TOB(1, 1);
    auto topdieinst2 = basedie.add_topdie_inst("topdieinst2", topdie2, &tob2);

    auto topdie3 = basedie.add_topdie("topdie3", std::HashMap<std::String, std::usize>{});
    auto tob3 = hardware::TOB(2, 2);
    auto topdieinst3 = basedie.add_topdie_inst("topdieinst3", topdie3, &tob3);
    
    topdieinstace_ptrs.push_back(topdieinst1);
    topdieinstace_ptrs.push_back(topdieinst2);
    topdieinstace_ptrs.push_back(topdieinst3);
    
    std::Vector<TopDieInstance*> topdieinstances;
    for (const auto& ptr : topdieinstace_ptrs) {
        topdieinstances.push_back(ptr);
    }
    ...
    for (const auto& topdie : topdieinstances) {
        if (topdie->tob()) {
...
//                 basedie->add_topdie(topdie_name, std::HashMap<std::String, std::usize>{});
